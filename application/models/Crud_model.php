...
class Crud_model extends CI_Model
{
    ...
    public function add_assigment($course_id)
    {
        $data['link'] = html_escape($this->input->post('link'));
        $data['course_id'] = $course_id;
        $this->db->insert('assigment', $data);
        $assigment_id = $this->db->insert_id();

        
    }

    public function edit_assigment($assigment_id)
    {
        $data['link'] = $this->input->post('link');
        $this->db->where('id', $assigment_id);
        $this->db->update('assigment', $data);
    }

    public function delete_assigment($assigment_id)
    {
        $this->db->where('id', $assigment_id);
        $this->db->delete('assigment');
    }

    public function get_assigment($type_by, $id)
    {
        $this->db->order_by("id", "asc");
        if ($type_by == 'course') {
            return $this->db->get_where('assigment', array('course_id' => $id));
        } elseif ($type_by == 'assigment') {
            return $this->db->get_where('assigment', array('id' => $id));
        }
    }

    public function serialize_assigment($course_id, $serialization)
    {
        $updater = array(
            'assigment' => $serialization
        );
        $this->db->where('id', $course_id);
        $this->db->update('course', $updater);
    }
    ...
    public function count_total_rating($ratable_type = "") {
        if($ratable_type == "course"){
            $query = $this->db
                ->select("AVG(rating) as average, ratable_id")
                ->from ("rating")
                ->group_by('ratable_id')
